module Sonowz.Raytrace.RTConfigCreator
  ( Config(..)
  , jsonToConfig
  )
where
...
checkConfigIntegrity conf =
  bound pixelWidth 2 500 "Pixel width"
    ...
    . bound sceneNo     1   3    "Scene number"
    $ Nothing where
  bound :: (Show a, Ord a) => (RTConfig -> a) -> a -> a -> String -> Maybe String -> Maybe String
  bound _field _min _max _text (Just message) = Just message
  bound _field _min _max _text Nothing        = if (_min <= _field conf) && (_field conf <= _max)
    then Nothing
    else Just $ _text ++ " must be in range [" ++ show _min ++ ", " ++ show _max ++ "]."
...
jsonToConfig _json = case decode _json of
  Nothing       -> DecodeFail
  Just rtConfig -> case checkConfigIntegrity rtConfig of
