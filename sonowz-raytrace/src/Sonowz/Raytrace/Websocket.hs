...
catchConnError wsdata id work = catch work (handler wsdata id) where
  handler :: (MonadIO io) => WSData -> Int -> WS.ConnectionException -> io a
  handler wsdata id e =
...
getParameters :: MonadIO io => WS.Connection -> io LB.ByteString
...
makeConfig :: MonadIO io => LB.ByteString -> io String
...
registerEvent :: MonadIO io => WSData -> String -> io (MVar WSMessage, Int)
...
unregisterEvent :: MonadIO io => WSData -> Int -> io ()
...
doProxy :: MonadIO io => WS.Connection -> MVar WSMessage -> io ()
...
closeRequest :: MonadIO io => WS.Connection -> io ()
...
send :: MonadIO io => WS.Connection -> String -> io ()
