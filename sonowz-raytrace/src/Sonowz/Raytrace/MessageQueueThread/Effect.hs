...
runMQueueStream :: Members [Time, MessageQueue rx, MessageQueue tx] r
  => StreamHandler r rx tx -> Sem (MessageQueueStream rx tx : r) a -> Sem r a
runMQueueStream handler = interpret $ \case
  DoStreamLoop -> runStreamLoop handler
    

runStreamLoop :: Members [Time, MessageQueue rx, MessageQueue tx] r
  => StreamHandler r rx tx -> Sem r ()
