...
sendSignal signal dictRef pid = getMVarFromDict dictRef pid >>= flip putMVar signal
...
doRaytraceProcess pid config = do
  ...
  case shellResult of
    Script.ShellResult ExitSuccess _ _ ->
      putTextLn (time <> ": Job #" <> show pid <> " finished.") >> return True
